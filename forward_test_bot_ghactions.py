...
def detect_new_bos_setup(df, i):
    ...
    if psh is not None and C[i] > H[psh]:
        # FRESHNESS CHECK: only count this as a NEW break of structure if
        # candle i is the FIRST candle since the swing high to close above
        # it. Without this, price sitting above the level for several bars
        # in a row would re-trigger a "new" setup on every single one of
        # them (duplicate signals with identical entry/stop/target).
        if not np.any(C[psh+1:i] > H[psh]):
